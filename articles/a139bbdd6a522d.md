...
また、これがCoeffectと呼ばれる型システムの一例であることを述べます。
...
なお、この節から次の節までのコードは、説明用の擬似コードです。実際の言語のコードは「具体例：`url` と `body`」から登場します。
...
ここからは実際に僕が作っている言語 [Katari](https://katari-lang.dev) のシグネチャを使って解説していきます。
...
分配束は、joinを`×`、meetを`+`、さらに`⊑`を順序として見れば**順序付半環**になります（双対に取ることもできます）。
